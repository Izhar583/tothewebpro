...
function FaqAccordionItem({ question, answer }: { question: string; answer: string }) {
  return (
    <details className="group border border-orange-100 rounded-2xl bg-white overflow-hidden transition-all duration-200 shadow-sm hover:border-orange-200">
      <summary className="w-full flex items-center justify-between p-5 text-left font-bold text-slate-800 hover:text-orange-600 transition-colors cursor-pointer list-none [&::-webkit-details-marker]:hidden">
        <span>{question}</span>
        <ChevronDown className="h-5 w-5 text-slate-400 shrink-0 transition-transform duration-200 group-open:rotate-180 group-open:text-orange-500" />
      </summary>
      <div className="border-t border-orange-50/50 p-5 text-sm text-slate-600 leading-relaxed font-medium bg-orange-50/10">
        {answer}
      </div>
    </details>
  );
...
export function MetaCheckerLandingPage() {
  return (
